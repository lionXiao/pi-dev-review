...
- **报告 JSON 括号修复（默认开）**：子 agent 有时写完整份报告却漏掉 `]`/`}`（典型：`resolved_issues` 元素写完后忘了关数组，后续 `tests/assumptions/…` 全被裹进最后一个元素里），而 `stopReason` 是正常的 `stop`——不是截断，引擎以前直接升级成人处理并烧掉一轮。现在解析失败时会尝试**唯一解**的括号补齐：只在某个顶层字段前插入缺失的闭合符后能解析、且所有报告字段都留在顶层、且不会产生重复顶层键时才采用；零个或多个候选一律照旧升级。修复永远只在原地插字符，原样输出存到 `handoffs/<role>-rNN.raw.txt`、审计记录写 `handoffs/<role>-rNN.repair.json`，timeline 追加一行 `report-repaired`，handoff 末尾附「Report JSON auto-repaired」说明。`DEV_REVIEW_REPORT_REPAIR=0` 关闭。输出**真的**在容器中途断掉（后面没有内容）时不修——那种情况分不清“漏括号”和“被截断”，仍然交给人。
- **报告被拒会重试**：最终消息解析不了或过不了 schema 时，引擎在同一 session 重试 `protocolRetries` 次（默认 1，`--protocol-retries`），把失败原因回贴给 agent 并要求只重出报告；仍失败才升级给人。每次被拒的原文存 `handoffs/<role>-rNN[-attemptN].raw.txt`，timeline 记一行 `report-retry`。
- **报告/执行失败不吃轮次预算**：非成功产出的轮次计入 `state.reportFailures`，`roundBudget = maxReviewRounds + 失败数`；`Round:` 行显示为 `3/11 (1 report failure(s) excluded)`，max-rounds 升级摘要也会说明排除了几轮。
