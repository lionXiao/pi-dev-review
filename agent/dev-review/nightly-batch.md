# 夜跑批处理协议（Nightly Batch）

> 触发：用户说「夜跑」并给出队列文件（默认 `.ai-dev-review/nightly-queue.md`）。
> 目标：一个 pi 会话内串行跑完队列里所有 plan；中途**不找用户**，早上一次性汇总与排查。
> 用户授权日期：2026-09-13（规则来源：用户口述，见 §1）。

## 1. 授权边界（最重要）

用户给出队列文件 = **预授权**主 agent 在夜间按本协议代为决策。仅限：
...
```
夜间预授权（队列 .ai-dev-review/nightly-queue.md，<日期>），无人工当场确认。
依据：<分类表条目>。<具体处理方式>
```
...
- [ ] 队列文件存在且每行格式合法（见 §6）
- [ ] 没有 `running` 实例（`/dev-review list`）；有 blocked 旧实例 → 记入夜报「遗留」节，不阻挡开跑
- [ ] 工作树干净（`git status`）。不干净 → 先 commit/stash 或等用户，**不得**用 `--allow-dirty` 绕过
- [ ] 队列内每份 plan 都已冻结（夜跑期间不改 plan；改了 = 新实例）
- [ ] 创建夜跑分支：`git checkout -b nightly/<YYYY-MM-DD>`（从当前 HEAD）
...
每份 plan 的自动 resolve 上限 **2 次**；超过即 SKIP，写进夜报，不再纠缠。
...
4. 该 plan 收尾（git 卫生，保证下一份从干净可构建的基础开工）：
   - **pass**：把工作树提交到夜跑分支：`nightly: <key> pass (pending morning review)`；
   - **skip / blocked / 异常**：把当前工作树提交到 `nightly/<date>-<key>`（park，注明「未评审 WIP」），
     再把夜跑分支 `git reset --hard` 回上一份 pass 的 commit。
     这样半成品不会把后续 plan 的构建/审查污染掉；被 park 的 WIP 早上可单独恢复。
5. 追加夜报一节（§7），进入下一项。
6. **熔断**：当前时间超过 cutoff（默认 07:00，可在队列文件覆盖）→ 停止队列，直接写夜报。
   不要在早上把用户没睡够的时间用来跑长 plan。

## 5. 同一 plan 内未完成的部分

不追加轮次硬啃。夜报里按三栏记录（从 dev/reviewer 报告与 escalation 里提取）：

- 已完成：<验收项/批次>
- 未完成：<验收项/批次>（具体到条目）
- 阻塞点：<一句话> + escalation 路径

## 6. 队列文件格式（`.ai-dev-review/nightly-queue.md`）

```md
# 夜跑队列 2026-09-13

授权：用户于 2026-09-13 预授权，按 ~/.pi/agent/dev-review/nightly-batch.md 执行。
cutoff: 07:00
...
## 7. 夜报格式（`.ai-dev-review/nightly-<date>.md`）
...
1. 先读夜报，按「建议排查顺序」逐份处理
2. blocked 的 plan：`/dev-review use <key>` 切为活动实例 → `dev_review_resolve` → `dev_review_run`
3. 被 park 的 WIP（`nightly/<date>-<key>` 分支）：建议在独立 `git worktree` 里恢复续跑，避免与后续 plan 的改动互染
4. 验收夜跑分支：每份 plan 一个 commit，边界即 plan 边界
5. 全部处理完再决定合并/丢弃/续跑

## 9. 已知限制（如实告知用户）

- 会话退出、机器睡眠 = 唤醒链断（状态都在磁盘，早上可手动续）
- 一段夜跑只在一个工作树里串行；真正的跨工作树并行需要 CLI 驱动，会失去主 agent 的判断力
- 夜里不会推送手机（未配置 notify.json）；桌面通知在窗口失焦时可能出现
- 被 SKIP 的 plan 不会被夜跑自动回头处理，只做标记
