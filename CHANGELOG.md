...
- **两段补偿文本改为纯英文**：`STALL_DIRECTIVE`（给 developer，标题 `## Stall-family acceptance criteria (engine-injected)`）与 `STALL_CANDIDATES`（给 reviewer，标题 `## Unlinked-candidate confirmation (engine-injected)`）。依据是仓库已有的语言分工：`prompts/developer.md` / `prompts/reviewer.md` 明确“**指令英文，人类可读内容跟随冻结 plan 语言**”，而这两段是引擎注入的**判据**（指令），不是内容；中文-only 曾在英文任务包里可能被弱化遵循，而双写会变成同一判据的两份真相——一旦漂移没人知道模型读了哪半。
- **数据行同步去中文**：家族概览只剩英文一行（`family R1-003: 4 linked findings over the last 3 review rounds, 4 still open`），成员行 `· first r1 · last r4: partially_fixed · open`，候选行 `· first r19 · new this round`。家族表里 LLM 写的 `requirement` 文本仍跟随 plan 语言（不翻译、不丢信息）；给人看的硬升级家族报告（中文）未动。
- **占位符替换改走函数 replacer**：`{families}` / `{family}` / `{candidates}` 改用 `replaceAll(..., () => text)`——原文里的 `$&`、`$1`、`$'` 这类序列会被字符串形式的替换当成捕获引用改写（候选行的 `requirement` 是 LLM 写的，可能带 `$`）。双写阶段 `{family}` 曾出现两次而 `String.replace` 只能换第一处，这个改动顺便消除了该漏替换。
- 测试同步：既有断言里的中文标记字串改为英文标记（`Stall-family acceptance criteria` / `Unlinked-candidate confirmation`）；新增 3 条断言——两段模板均无中日韩字符（英文约束不被意外写回中文，可在 diff 阶段拦住漂移）、占位符全部被替换（`repeat_of` 恰好 1 次）、`$&`/`$1`/`$'` 文本原样保留。全套 76/76 通过；`install.sh` 已重装到 `~/.pi/agent`（两处 sha256 一致）。

### 文档：README 补齐停滞检测与无人值守口径
