...
- **占位符替换改走函数 replacer**：`{families}` / `{family}` / `{candidates}` 改用 `replaceAll(..., () => text)`——原文里的 `## Unreleased — 时间线升级为全过程日志`、`$1`、`
...
` 这类序列会被字符串形式的替换当成捕获引用改写（候选行的 `requirement` 是 LLM 写的，可能带 `# Changelog

）。双写阶段 `{family}` 曾出现两次而 `String.replace` 只能换第一处，这个改动顺便消除了该漏替换。
- 测试同步：既有断言里的中文标记字串改为英文标记（`Stall-family acceptance criteria` / `Unlinked-candidate confirmation`）；新增 3 条断言——两段模板均无中日韩字符（英文约束不被意外写回中文，可在 diff 阶段拦住漂移）、占位符全部被替换（`repeat_of` 恰好 1 次）、`## Unreleased — 时间线升级为全过程日志`/`$1`/`
...
设计决策见 [docs/adr/0001-discipline-injection.md](docs/adr/0001-discipline-injection.md)。
` 文本原样保留。全套 76/76 通过；`install.sh` 已重装到 `~/.pi/agent`（两处 sha256 一致）。
